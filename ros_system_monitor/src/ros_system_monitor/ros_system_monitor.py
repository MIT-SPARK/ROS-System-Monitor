...
def info_to_row(info: TrackedNodeInfo):
    ...
    status_to_color = {Status.NOMINAL: "green",
...
class SystemMonitor(Node):
    def __init__(self, config_path):
        ...
        for n in self.config.nodes_to_track:
            info = NodeInfo(nickname=n, node_name="???", status=Status.NO_HB, notes="Not yet seen")
    ...
    def update_node_info(self, info):
        ...
        with self.info_lock:
            ...
            if info.nickname in tracked_nodes:
                ...
                self.get_logger().error(f"Received diagnostics from untracked node: {info.nickname} ({info.node_name})")
